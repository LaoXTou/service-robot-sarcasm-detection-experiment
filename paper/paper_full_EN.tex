...
\textbf{Data source.} All experimental data was generated by DeepSeek V4-Flash
across 10 service interaction scenarios (restaurant, food delivery, hotel,
ride-hailing, express delivery, e-commerce, hospital, supermarket, banking, smart
home). For each scenario, we defined 6--8 typical failure modes (e.g., for
restaurant: ``food delayed 40 minutes,'' ``dish-photo mismatch,'' ``serving robot
spill'') and instructed the LLM to generate user expressions in these contexts.
Generation parameters: temperature=0.3, frequency\_penalty=0.2. The system prompt
enforces four quality rules: no fabricated words, no full sentences as keywords,
no academic jargon, and pure JSON output.

\textbf{Label definitions.} Table~\ref{tab:labels} defines the four-class schema.

\begin{table}[H]
\centering
\caption{Four-class label definitions with criteria and examples.}
\label{tab:labels}
\begin{tabular}{llp{5cm}}
\toprule
\textbf{Label} & \textbf{Meaning} & \textbf{Criterion \& Example} \\
\midrule
Neutral(0)  & Objective description or request & No sentiment reversal. ``Please deliver to Table 3.'' \\
Positive(1) & Explicit praise or satisfaction & Both literal and true intent are positive. ``The service was timely, thank you.'' \\
Sarcasm(2)  & Literally positive, truly negative & Semantic inversion or contextual contradiction. ``Your speed is really fast!'' (to a stuck robot) \\
Anger(3)    & Direct negative complaint & No irony needed. ``Terrible, I want to file a complaint.'' \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Split and deduplication.}
The source dataset contains 8,000 LLM-generated service-scenario texts, from
which we performed complete text deduplication followed by a fixed stratified
...
all experiments, never participating in training, validation tuning, or retrieval
threshold selection. Table~\ref{tab:data} summarizes the label distribution.
...
LLM-generated data passed through a multi-agent scoring pipeline: DeepSeek V4-Pro
assigns literal ($l_t \in [-5,5]$) and physical-context ($p_t \in [-5,5]$)
sentiment scores, and DeepSeek V4-Flash audits against three red-line rules
(polarity conflict, logic inversion, insufficient intensity). Entries are retried
up to 3 times, achieving 99.7\% Approved status. Additionally, 100 randomly
sampled entries were independently verified by three lab members for four-class
label consistency, yielding Fleiss' $\kappa = 0.81$ (substantial agreement);
7 entries with annotation ambiguity were corrected.

\textbf{Limitation note.}
All data is LLM-generated; models may learn generator-specific stylistic features
rather than generalizable sarcasm understanding. This is an explicit limitation
discussed further in \S 7.
