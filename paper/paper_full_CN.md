...
本文贡献如下：
1. 一个三源异构常识知识库，包含 11,508 条条目，覆盖 10 个服务交互场景和 4 种情感类别，专门面向服务机器人指令语义理解
2. 双路混合检索机制（精确子串匹配 + FAISS 余弦阈值过滤），检索 F1 达到 0.83，优于任一单路策略
3. BERT-Base-Chinese 消融实验，验证知识增强使 4 种子准确率均值从 66.12% 提升至 86.31%（+20.19pp），反讽召回率从 67.7% 提升至 93.9%（+26.3pp），所有种子均观察正向增益
...
实验数据全部由 DeepSeek V4-Flash 生成，覆盖 10 个服务交互场景（餐厅、外卖、酒店、网约车、快递、电商、医院、超市、银行、智能家居），标注为四分类：中性（0）、正面（1）、反讽（2）、愤怒（3）。原始数据集包含 8,000 条 LLM 生成的服务场景文本（`diverse_service_train.json`），从中固定切分出训练集 3,579 条、验证集 400 条和独立测试集 400 条（`test_raw_400.json`）。训练/验证/测试三部分之间无任何文本重叠，测试集作为固定留出集在所有实验中保持不变。标签分布详见表 X。
...
所有数据均由 LLM 生成，标签分布反映生成 prompt 的意图分布。反讽类占比最高（~45-50%），愤怒类占比最低（~1.5-1.8%），是生成过程中的系统性偏差。
